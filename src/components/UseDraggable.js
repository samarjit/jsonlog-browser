...
/// use-draggable.ts
const id = (x) => {console.log(JSON.stringify(x)); return x;};
// complex logic should be a hook, not a component
const useDraggable = ({ onDrag, onDragEnd = id}) => {
  // this state doesn't change often, so it's fine
  const [pressed, setPressed] = useState(false);
  const [mouseDnPos, setMouseDnPos] = useState({x: 0, y: 0});
  ...
  const legacyRef = useCallback((elem) => {
    ...
    const handleMouseDown = (e) => {
      ...
      setMouseDnPos({x: e.pageX, y: e.pageY})
      setPressed(true);
    };
  ...
  useEffect(() => {
    ...
    const handleMouseMove = throttle((event) => {
      ...
      position.current = onDrag({
        ...
        x: event.pageX,
        y: event.pageY,
        relx: event.pageX - mouseDnPos.x,
        rely: event.pageY - mouseDnPos.y,
      });
      // elem.style.transform = `translate(${pos.relx}px, ${pos.rely}px)`;
    ...
    const handleMouseUp = (e) => {
      ...
      setPressed(false);
      const relx = e.pageX - mouseDnPos.x;
      onDragEnd({x: e.pageX, relx});
    };
